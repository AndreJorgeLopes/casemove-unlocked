import { getPriceKey } from '../../functionsClasses/prices';
  ...
  switch (action.type) {
    ...
    case 'PRICING_ADD_TO': {
      ...
      action.payload.itemRows.forEach((element) => {
        const key = getPriceKey(element, currentPrices);
        currentPrices[key] = element.pricing;
      });
    ...
    case 'PRICING_ADD_TO_REQUESTED': {
      const nextRequested = new Set(state.productsRequested);

      action.payload.itemRows.forEach((element) => {
        nextRequested.add(getPriceKey(element));
      });

      return {
        ...state,
        productsRequested: Array.from(nextRequested),
      };
    }
    case 'PRICING_REMOVE': {
